package com.dsa.tree.Problem.BinaryTree.hard;
...
public class Print_All_Nodes_At_Distance_Of_K {
	...
	public static List<Integer> getKthNodeFromTargetNode(TreeNode root, TreeNode targetNode, int position){
		...
		buildChildParentMap(root, childParentRelationMap);
		
		getKthPositionList(targetNode, position, childParentRelationMap, kthPostionList);
		return kthPostionList;
		
	}
	
	
	public static  void buildChildParentMap(TreeNode root, Map<TreeNode,TreeNode> childParentMap) {
		Queue<TreeNode> queue = new LinkedList<>();
		queue.offer(root);
	...
	public static void getKthPositionList(TreeNode targetNode, int kthPostion, Map<TreeNode,TreeNode> childParentMap, List<Integer> kthPostionList) {
		Queue<TreeNode> queue = new LinkedList<>();
		...
		while(!queue.isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                // Check parent
                if (childParentMap.containsKey(node) && !visitedNode.contains(childParentMap.get(node))) {
                    queue.add(childParentMap.get(node));
                    visitedNode.add(childParentMap.get(node));
                }
            }
            kthPostion --;
		}
